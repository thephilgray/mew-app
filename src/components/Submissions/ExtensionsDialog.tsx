...
import { createExtension as createExtensionMutation, deleteExtension as deleteExtensionMutation } from '../../graphql/mutations'
...
const ExtensionsDialog: React.FC<{ assignmentId: string, open: boolean, onCloseDialog: React.FC }> = ({ assignmentId = '', open = false,
  onCloseDialog }) => {
  const { data: { extensionsByFileRequestId } = {}, loading, error, refetch } = useQuery(gql(extensionsByFileRequestIdQuery), {
  ...
  useEffect(() => {
    if (extensionsByFileRequestId) {
      setExtensions(extensionsByFileRequestId?.items.filter(ex => !ex._deleted))
    }
      ...
      await API.graphql({
        ...
        variables: {
          input: {
            id: extensionId,
            // @ts-ignores
            _version: extensionToDelete._version
          },
        },
      })
    ...
    <>
      ...
      >
        ...
        <DialogContent>
          ...
          <DialogContentText>
            Secret links that give people a little more time to submit.
          </DialogContentText>
          <Grid container >
            {extensions.length > 0 && <Grid xs={12}>
                  ...
                  <TableBody>
                    {extensions.map((row) => (
                      <TableRow key={row.id}>
                        ...
                        <TableCell align="center">{format(new Date(row.expiration), 'MM/dd/yyyy H:mm')}</TableCell>
                        <TableCell align="right">
                          <IconButton
                            ...
                            onClick={() =>
                              copyToClipboard(
                                `${window.origin}${ROUTE_NAMES.newPublicSubmissionExtension.getPath({
                                  assignmentId,
                                  // @ts-ignore
                                  extensionCode: row.id
                                })}`,
              ...
              <Grid container alignItems="center" spacing={2}>
                <Grid item xs={5}>
                  <KeyboardDatePicker
                    ...
                    // @ts-ignore
                    onChange={(date) => setCurrentExtension(prev => ({ ...prev, expiration: date }))}
