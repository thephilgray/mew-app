...
export default function NewAssignment(): JSX.Element {
    const { register, handleSubmit, errors, setValue } = useForm<Inputs>()
    const [description, setDescription] = useState<string | null>('')
    const [startDate, setStartDate] = useState<Date | null>(new Date())
    const [dueDate, setDueDate] = useState<Date | null>(add(new Date(), { weeks: 1 }))
    ...
    useEffect(() => {
        ...
        register({ name: 'dueDate' }, { required: true })
    }, [dueDate])
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            <Grid container spacing={3}>
                ...
                <Grid item xs={12}>
                    <Editor
                        initialValue="<p>Assignment Details</p>"
                        init={{
                            height: 500,
                            menubar: false,
                            plugins: [
                ...
                <Grid item xs={6}>
                    <KeyboardDatePicker
                        ...
                        format="MM/dd/yyyy"
                        label="Due date"
                        helperText={!!errors.dueDate && <>Start date is required</>}
